...
4. Two options:
    - Copy your desired variant to `style.css` (i.e. `cp rose-pine.css style.css`)
    - Use swaync's `-s` flag to declare the variant on launch (i.e. `swaync -s ~/.config/rose-pine.css`)
...
```sh
npx @rose-pine/build@0.9.1 -t ./theme/style.css -o ./theme/ -p @ -f hex
```
